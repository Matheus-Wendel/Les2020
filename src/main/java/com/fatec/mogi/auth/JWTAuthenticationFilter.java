...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			com.fatec.mogi.model.domain.User creds = new ObjectMapper().readValue(req.getInputStream(),
					com.fatec.mogi.model.domain.User.class);
...
//	        writer.flush();
		...
		for (GrantedAuthority grantedAuthority : authority) {
			permission = grantedAuthority.toString();
		}
		String token = JWT.create()
				.withSubject(((org.springframework.security.core.userdetails.User) auth.getPrincipal()).getUsername())
				.withClaim("AUTH", permission)
				.withExpiresAt(new Date(System.currentTimeMillis() + SecurityConstants.EXPIRATION_TIME))
				.sign(Algorithm.HMAC512(SecurityConstants.SECRET.getBytes()));
		// res.addHeader(SecurityConstants.HEADER_STRING, SecurityConstants.TOKEN_PREFIX
		// + token);
		// res.addHeader(SecurityConstants.PERMISSION_STRING,
